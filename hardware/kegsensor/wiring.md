...
![KegSensor wiring: 4 load cell sensors through the KegSensor PCB (J1-J4),
HX711 (J5/J6), RJ11 (J7), out to KegHub](wiring_diagram.png)

Generated by `generate_wiring_diagram.py` (matplotlib) — regenerate after
editing that script with `python3 generate_wiring_diagram.py`. Every
signal path, pin label, and wire color in the diagram matches the tables
below directly, not hand-drawn separately from them — with one
deliberate exception: J6 is drawn as GND/VCC/SCK/DT (matching J7's order,
so the connecting lines run straight instead of crossing), not the PCB's
actual silkscreen order (GND/DT/SCK/VCC — see the Connectors table
below). The diagram shows logical wire connections, not a literal
pin-position map of the real header; check the Connectors table or the
physical board itself before wiring against J6, not this picture.
